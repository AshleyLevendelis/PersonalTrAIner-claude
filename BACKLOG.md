...
- [x] **TOOLS BECOMES ONE TIMER, AND THE SHOPPING LIST GETS ITS OWN SCREEN.**
  ...
  build the one in the zip.
  **TOOLS.** Six tiles, of which two opened the same panel in different modes
  and one ("Rest timer") pointed at a settings screen that has never existed,
  above a grocery section a fourth tile scroll-jumped to. Now: the round timer
  IS the tab — a card that colour-codes its phase — one row to change the
  intervals, one sentence saying the rest timer runs itself, and a short "Also
  here" list. **Full screen is opt-in**: a running round used to seize the whole
  tab until it was reset, with everything else unreachable mid-session.
  **THE SETUP IS CHIPS.** Three number boxes became a protocol row, a Work row,
  a Rest row, a stepper and a summary that says what is about to run and how
  long it takes. Rest "None" is the EMOM path the engine already ran. "Is this
  an EMOM" is now derived from that zero rather than stored beside it.
  **GROCERY LEFT TOOLS** for a route of its own under Nutrition — where it is
  built from — grouped by aisle with a count per heading, and checked items
  leave the aisles for one "In the trolley" row. On shop day it is also a card
  on Home, behind a `shopDay` preference that defaults to the day BEFORE the
  week's first training day (derived, not hard-coded to Sunday — for a Tue/Thu
  trainer that is Monday, and Sunday would put the card up two days early).
  Changeable in Profile, including handing it back to the app.
  **DEVIATIONS, both stated.** The handoff says keep RoundField unchanged;
  making full screen opt-in without a way out would make it a room with no
  door, so it gained one control to leave (the round keeps running). And it
  gives Work a Custom chip and says nothing about Rest; Rest got one too,
  because a 45-second rest is an ordinary prescription.
  **NOT BUILT, named:** the screen's "This week / Next week / + Add" chip row.
  Next week's list is not generated by anything, so two of the three would be
  controls that open nothing — the exact defect the retired "Rest timer" tile
  was. The screen-level "11 to buy · 3 in the trolley" line is also out: the
  aisle headings and the trolley row already carry those numbers, and a third
  place saying them is a third place to disagree.
  **NINE GATES RE-ANCHORED, NOT DELETED** — every one pinned to a mechanism the
  redesign moved rather than to the property it was written for:
  `tools-grid` (rewritten for the new surface; the subtitle length limit moved
  from 23 characters in a half-width tile to 40 in a full-width row, and the two
  numbers are NOT comparable), `round-presets`, `timer-field`,
  `timer-and-intent-copy`, `round-timer`, `nutrition-layout`, plus the browser
  drivers `verify:round-presets` and `verify:round-lead-in`.
  **AND FOUR THAT THE MEALS WORK BROKE**, found by the sweep and measured
  against origin/main to prove they were green there: `dashboard` (the chat
  meal handler was named, not deleted), `replace-without-losing` (the rollback
  moved into the shared executor), `timer-field-fills-screen` (matched
  `<RoundField />` including the punctuation), and `bundle`.
  **THE BUNDLE BUDGETS MOVED, stated:** app chunk 1,005 -> 1,030 kB, gzip 280
  -> 292 kB, total 1,740 -> 1,770 kB. MEASURED: 998 kB on origin/main, 1,015 kB
  after this branch's four pieces of work. +17 kB for editing one food in a
  meal on both surfaces, the Tools redesign and the grocery screen.
  **NEW GATES:** `test:shop-day` (8 mutations tried, 8 caught — including all
  three conditions dropped one at a time, a snooze that never expires, and
  unreadable storage treated as a snooze) and `verify:tools-timer` (a real
  2 × 4s / 2s round at 390x844: the settled tab, the card with the rest of the
  tab still reachable, full screen entered and left, and three phases sampled
  as three colours — 5 mutations tried, 5 caught).
  **TWO OF MY OWN CHECKS WERE WRONG BEFORE THE CODE WAS.** `verify:tools-timer`
  first sampled the "work" colour during the ten-second countdown, so a
  mutation painting the finished card in the working colour passed against the
  grey ready one. And `verify:round-presets` matched the summary line by its
  text and found the EMOM preset CHIP instead — `describeRoundPreset` writes
  "10 × every 60s" too — so three checks compared a string with itself.
  **FOUND BY READING A SCREENSHOT**, not by any assertion: the setup panel
  stayed mounted under a running round, putting a tab strip beneath the card.
