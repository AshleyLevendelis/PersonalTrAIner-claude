...
const chrome=spawn('/opt/pw-browsers/chromium',['--headless=new','--remote-debugging-port=9342','--no-sandbox','--disable-gpu','about:blank'],{stdio:'ignore'})
const wait=ms=>new Promise(r=>setTimeout(r,ms)); let t
for(let i=0;i<60;i++){try{const l=await fetch('http://127.0.0.1:9342/json/list').then(r=>r.json());const g=l.find(x=>x.type==='page');if(g){t=g.webSocketDebuggerUrl;break}}catch{} await wait(250)}
...
for (const k of KEYS) {
  await ev(`document.querySelector('[data-q="${k}"]').scrollIntoView({block:'start'})`)
  await wait(350)
  const r = await send('Page.captureScreenshot',{format:'png'})
  writeFileSync(new URL(`./q-${k}.png`, import.meta.url).pathname, Buffer.from(r.result.data,'base64'))
  console.log('  wrote q-' + k + '.png')
}
