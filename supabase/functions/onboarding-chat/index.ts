...
- They answered in free text and the mapping is CERTAIN ("just some dumbbells at home" → equipment=minimalist, because minimalist already assumes dumbbells, kettlebells, bands, a pull-up bar and a weighted bag; home_gym assumes a barbell, a rack and a bench on top of all that) → call set_slot with the exact allowed value. The app shows them what was recorded — never map silently in your head and move on without the call.
